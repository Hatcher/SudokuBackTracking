...
public class Grid{
  ...
  public LinkedList<Arc> constrainSquare(int col, int row, Cell[][] grid, LinkedList<Arc> queueConstraints){
      ...
      for (int j = topLeftCol; j < topLeftCol+2; j++) {
        if( i != row && j != col){ //If this is not the same row or column as the cell
          ...
          */
          ...
          Arc newConstraint = new Arc(grid[i][j], grid[row][col]);
  ...
  public LinkedList<Arc> constrainRow(int col, int row, Cell[][] grid, LinkedList<Arc> queueConstraints){
    for(int i = 0; i < 9; i++){
      if(i != col){
        boolean arcExists = false;
        Arc newConstraint = new Arc(grid[row][i], grid[row][col]);
        for (int q = 0; q < queueConstraints.size(); q++) {
          if(queueConstraints.get(q).equals(newConstraint)){
            arcExists = true;
          }
  ...
  public LinkedList<Arc> constrainCol(int col, int row, Cell[][] grid, LinkedList<Arc> queueConstraints){
    return this.constrainRow(row, col, grid, queueConstraints);
  }
